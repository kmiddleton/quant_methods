...
stalk %>% 
  ...
  summarise(var_eye_span = var(eye_span))
```
...
> The `(Intercept)` term from `lm()` is equal to the mean for the Corn group. The `food_sourceCotton` estimate is the *difference* between the means for the Corn and Cotton group means. $F = t^2$.
...
> There is overwhelming support for the two mean model. This is pretty apparent from the plot.

We have actually violated one of the assumptions of the likelihood calculations that we did. What is it?

> The variances are likely *not* equal. The variance is 16x higher in the cotton group than in the corn group.
