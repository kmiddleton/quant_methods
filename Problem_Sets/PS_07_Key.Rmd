...
ggplot(sims, aes(sd_both, Fs)) +
  geom_point()


fcrit <- qf(0.05, 1, 98, lower.tail = FALSE)

ggplot(sims, aes(x = sds, y = Fval)) +
  geom_point() +
...
M %>% head()
M %>% summarize_each(funs = c("mean", "sd"))
```
...
```{r}
# FIXME
fm <- lm(N_Species ~ Dist_to_Island + Elevation + Area + Dist_to_Ecuador, data = M)
```
