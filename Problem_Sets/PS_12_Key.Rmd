...
10^5^ iterations took ~84 seconds on a single processor (on this computer). We don't need to run that many iterations to get a good sense that the empirical P-value is going to be very low. The analytical *P* is 6^-10^, so we expect the empirical *P* to be on the same scale.
