...
Look back at the plot you just made. It appears a linear fit doesn't fit the data very well. Growth rate appears to be flat for early days and then increases exponentially. In addition, the linear fit is predicting negative biomasses for early timepoints. Let's transform our data to see if we can improve the fit. Make a new plot and add the geom `scale_y_log10()` to plot the y-axis on a log10 scale. 
...
> Yes, the fit looks much more appropriate now. 
...
To visualize whether there are consistant differences in richness between rainout treatments, we will plot our data. 
...
ggplot(richness, aes(x = rain_removal, y = richness, color = factor(plot_no))) +
...
# Return to Spider Size Dimorphism
