...
When you are working with standalone .R files, it can be useful to set the working directory with the `setwd()` function. This is best to be avoided when working with .Rmd files. When you knit an Rmd by clicking the "Knit" button, RStudio starts a new R session and sets the working directory to the directory containing the file you are knitting. Thus if you use `setwd()` within a R chunk, you will get a warning. Better practice when working with an Rmd file interactively is to set the R working directory using the menu commands: **Session $\rightarrow$ Set Working Directory $\rightarrow$ To Source File Location**. This way, you will be able to work interactively in RStudio and the relative paths will be the same as when you knit.
