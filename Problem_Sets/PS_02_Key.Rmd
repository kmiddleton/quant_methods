...
- When you are working with `.Rmd` files and click "Knit" RStudio starts a new R session in the directory that the Rmd file is located. Thus if you use `setwd()` within a R chunk of an Rmd file, you will get a warning. Better practice when working with an Rmd file interactively is to set the R working directory using the menu commands: **Session $\rightarrow$ Set Working Directory $\rightarrow$ To Source File Location**. This way, you will be able to work interactively in RStudio and the relative paths will be the same as when you knit.
