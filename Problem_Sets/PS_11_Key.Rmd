...
- Use the `names_prefix =` option to prefix "Measurement_1" before the column name
...
ex.dat <- tibble('treatment' = rep(c('a', 'b'), each = 20),
                 'yy1' = rnorm(40),
                 'yy2' = rnorm(40))

# Extract the predictor as a vector
treat <- ex.dat$treatment

# Extract the outcome y's as a matrix (40 rows x 2 columns)
YYs <- as.matrix(ex.dat[, 2:3])

# Fit separate models to both set of outcome variables
mods <- lm(YYs ~ treat)
