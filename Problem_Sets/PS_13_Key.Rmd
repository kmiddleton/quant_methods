...
# Note center = TRUE is the default but scale. = TRUE is not to make the results compatible with S. You should always use scale. = TRUE
...
a. Keeping `rho`, `sigma`, and `n` constant, how does changing `mu` influence the proportion of variance accounted for by `V1` and `V2`? Briefly explain.

> No change, because the data are centered prior to PCA.

b. Keeping `rho`, `mu`, and `sigma` constant, how does changing `n` influence the proportion of variance accounted for by `V1` and `V2`? Briefly explain.

> No change, because the correlation does not depend on `n`.

c. Keeping `rho`, `mu`, and `n` constant, how does changing `sigma` influence the proportion of variance accounted for by `V1` and `V2`? Briefly explain.

> No change, because the data are scaled prior to PCA.

d. Keeping `mu`, `sigma`, and `n` constant, how does changing `rho` influence the proportion of variance accounted for by `V1` and `V2`? Be sure to explore the full possible range of `rho`. Briefly explain.

> As `rho` increases, the proportion of variable accounted for by the first PC increases.


KEVIN - LOOP THROUGH SOME VALUES, KEEP & MAKE PLOT?

## 2. Return of the Sole

In Week 8, we looked at presence/absence data for the common sole ([*Solea solea*](https://en.wikipedia.org/wiki/Common_sole)) predicted by different factors such as the salinity of the water and the season. Today we will focus on the composition of the substrate.  
...
b. The columns `gravel`, `large_sand`, `med_fine_sand`, and `mud` represent their proportional concentrations in the substrate (they sum to 100). Calculate a composite score for "substrate". Use principal components analysis on these columns.
...
c. Use the `summary()` and `print()` methods to determine the percent of variance accounted for by each PC and the relative loadings of each variable.
...
d. Fit a logistic regression where presence of sole is predicted by the first PC from the PCA in part b. Refer to the lecture slides if you need to.
...
e. Try to interpret the output of the summary of the fitted model from part d. Why kind of substrate do sole appear to prefer?
...
The file `Egyptian_Skulls.xlsx` contains 4 measurements of 150 skulls from 5 time periods of Egypt from Thompson and Randall-Maciver (1905). Here is an image of the measurements that I found online (with typos):

![Skull Measurements](http://i.imgur.com/KrKDPGX.jpg)
...
a. Load the file. Convert `Time_Period` to a factor. Group by `Time_Period` and use `tally()` to confirm that there are 30 skulls per time period. Also, relevel the time periods so they go in chronological order.

```{r}
# FIXME
M <- read_excel("../data/Egyptian_Skulls.xlsx")
M <- M %>% mutate(Time_Period = factor(Time_Period))
...
b. Use MANOVA to predict `Maximum_Breadth`, `Basibregmatic_Height` `Basialveolar_Length`, and `Nasal_Height` from `Time_Period` using the `lm()` function (see the lecture notes). 
...
c. Generate the MANOVA table with `Anova()` using type III sums of squares.
...
d. What is your interpretation of the results of the MANOVA?

> Collectively the multivariate measurements vary by time periods.

e. We can also flip the analysis around and try to classify skulls by time period by their measurements. Use discriminant function analysis (linear discriminant analysis) to classify skulls by time period. See the lecture notes.
...
f. Calculate the percent correct classification for the DFA using the full dataset.
...
g. Generate the confusion matrix for the classification. Are there any patterns as to which time periods are best or worst predicted?
...
h. Now use the `CV=TRUE` option to perform leave-one-out cross validation (see the help for `lda()`). 
...
a. Load the file `pottery.xslx` and check the structure, and create a new object having dropped the column `kiln`. This will leave you with 9 columns of data in your new object. We will return to the `data.frame` with `kiln` later.
...
KEVIN - I HAD TO SPECIFY DPLYR TO GET THIS TO GO.

b. Calculate the principal components using centered and scaled data.
...
c. Using the `summary()` output: (1) what percentage of the variance does PC1 account for? (2) What percentage of the variation do the first 5 PCs cumulatively account for?
...
d. Using the `print()` method for a `prcomp` object, (1) what variables load positively on PC1? (2) What variables load negatively on PC1? (3) What variables have minimal loading on PC1?
...
e. Make a biplot of PC2 vs. PC1. What variables have similar loadings on these two axes?
...
f. Return to the full dataset including the `kiln` column. Convert `kiln` to a factor
...
g. Perform discriminant function analysis on the kiln column predicted by the first 9 variables. Calculate the percent correct classification for the full set (as for question 2, part f).
...
h. Calculate the confusion table for the classification.
...
i. Use the `CV = TRUE` option to perform cross validation and calculate the % correct and confusion table as in question 2, part h. 
...
KEVIN - SHOULD WE DO THIS EXAMPLE WITH A LOGISTIC REGRESSION TO SHOW COMPARISION?
