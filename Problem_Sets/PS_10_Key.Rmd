...
Use `lme()` and be sure to include the option `method = "ML"` to use maximum likelihood. Save both models to objects and take a look at the summaries to make sure that you are getting output that makes sense. 
...
> The second model is massively preferred ($\Delta$AIC = 2560). This makes sense because there is a lot of day to day variation in temperature. We could probably do even better by adding a main effect of day to model the overall trend of increasing temperature.

Fit a third model that is the same as model 2, but with the addition of `Day` as an additive, main effect. This model fits an overall slope to `Day`, with the random effects slope now an offset from the overall slope. Then use `aictab()` to compare all three models.
...
pred <- crossing(ln_Mass = seq(3.9, 4.5, length.out = 100),
  ...
  mutate(ln_Brain_pred = predict(fm2, newdata = .))
