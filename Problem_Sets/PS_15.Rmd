...
```{r}
d_tx <- seq(-10, 10, length.out = 10)
d_int <- seq(-10, 10, length.out = 10)
ds <- crossing(d_tx, d_int)
```
...
s0t0 <- 0
s1t0 <- 0
s0t1 <- as.numeric(ds[1, "d_tx"]) + 0.5 * as.numeric(ds[1, "d_int"])
s1t1 <- as.numeric(ds[1, "d_tx"]) + -0.5 * as.numeric(ds[1, "d_int"]) 
c(s0t0, s1t0, s0t1, s1t1)
```
...
n <- 5
sdd <- 4
y_s0t0 <- rnorm(n, s0t0, sdd)
y_s1t0 <- rnorm(n, s1t0, sdd)
y_s0t1 <- rnorm(n, s0t1, sdd)
y_s1t1 <- rnorm(n, s1t1, sdd)

d <- data.frame(
  Ca = c(y_s0t0, y_s1t0, y_s0t1, y_s1t1),
  Sex = factor(c(rep(0:1, each = 5), rep(0:1, each = 5))),
  Tx = factor(rep(0:1, each = 10))
)
```
