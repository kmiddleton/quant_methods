...
pmap_dbl(ap_test, myfun) # Note that this is the same as above
```
...
pmap_dbl(ap_test_reorder, myfun) # Note that this is the same as above
```
...
```{r}
pmap_dbl(ap_test, function(x1, x2, x3) (x1 * x2) / x3)
...
> As the standard deviation increases, the $F$-statistic decreases.
