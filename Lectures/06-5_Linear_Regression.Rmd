...
This is a flat line ($\theta_0 = 0$) through the mean of $Y$.
...
## Likelihood from linear regression with $\theta_0 = 0$
...
Function to calculate the log-likelihood. Input the slope ($\theta_0$) and observed values of $X$ and $Y$. Return the log-likelihood.
...
For a range of $\theta_0$ from -10 to 10 in increments of 0.01, calculate the log-likelihood of the model. Save to a variable called `lls`. Then find the maximum value of `lls`, the associated $\theta_0$ and log-likelihood.

```{r}
theta_0 <- seq(-10, 10, by = 0.01)
lls <- numeric(length(theta_0))
for (ii in 1:length(theta_0)) {
  lls[ii] <- log_lik(theta_0[ii], X, Y)
}
...
theta_0_hat <- theta_0[which.max(lls)]
```
...
plot(lls ~ theta_0,
     ...
     xlab = "Slope (b)",
     ylab = "log-Likelihood")
points(theta_0_hat, max.ll, col = "red", pch = 16, cex = 1.5)
text(-10, -60, paste("theta_0 =", theta_0_hat, "\nlogLik =", round(max.ll, 2)),
     pos = 4, cex = 2)
```
