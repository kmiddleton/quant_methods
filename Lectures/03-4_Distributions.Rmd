...
Probability of observing exactly 1.7?

```{r echo=FALSE, message=FALSE}
ggplot(M, aes(x = x)) +
  geom_histogram(aes(y = ..density..), fill = "gray") +
  geom_line(data = M_prob, aes(x = Y, y = prob)) +
  geom_point(aes(x = 1.7, y = normal(1.7)), color = "red") +
  annotate("text", x = 2.1, y = normal(1.7) + 0.03,
           label = round(normal(1.7), 3), color = "red") +
  xlim(c(-3, 3))
```
