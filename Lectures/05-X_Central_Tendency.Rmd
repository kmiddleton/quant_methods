...
Start at observation #1 ($i = 1$) and add up the successive values of $Y$, where $Y_i$ refers to the *i*th observation of $Y$. Then divide that sum by $n$ observations

Usually the $i = 1$ and $n$ are left out of the summation notation:

$$\bar{Y} = \frac{\sum Y_i}{n}$$

The bar over $Y$ denotes a mean.
...
`c()` is the combine function: make a vector of the enclosed numbers or strings (or any R object).

## Mean undulation rate in a sample of flying snakes
...
The median is the middle measurement of a sample (the 50th percentile and the 0.50 quantile). If $n$ is even, then the median is the mean of the two middle observations.
