...
## Why distribution free methods?      
  ...
  - Very flexible
  - Uses only infomation from your dataset
      - Good & Bad
...
## Resampling
  ...
  - Sample from the data to estimate the variablility in the parameter estimate
...
ggplot(BeakLength, aes(x=BeakLength)) +
  ...
  geom_vline(xintercept = mean(BeakLength$BeakLength), color='coral',lwd=3)
...
bs1 <- sample(BeakLength$BeakLength, replace=T)
cat(bs1[1:6], "...")
...
for(jj in 1:100)
{
  BS[jj,'BS']<- mean(sample(BeakLength$BeakLength, replace=T))
}
...
## Applications & Resources: Resampling
  ...
  1. Bootstrapping in phylogenetics: Sample character vectors, construct best tree, repeat. Felsenstein 1985 Felsenstein and Kishino (1993)

  1. Jackknifing in quantitative genetics
    Roff papers:
    Roff & Preziosi 1994: The estimation of the genetic correlation: the use of the jackknife
    Roff 2002: Comparing G matrices: A MANOVA approach
    Roff 2008: Comparing sire and dam estimates of heritability: jackknife and likelihood approaches
...
set.seed(903)
dat<-data.frame('BeakLength' = c(rnorm(20,12,0.5),rnorm(20,13.5,0.5)), "Original" = rep(c('A','B'),each=20), "Type" = rep(c('A','B'),each=20))
...
ggplot(dat, aes(x=Type, y=BeakLength, color=Original)) +
  geom_point(position = position_jitter(width = 0.05), alpha=1/2, size=2)
```
...
```{r echo=FALSE, fig.height=4}
dat.s<-data.frame('BeakLength' = dat$BeakLength, "Original" = dat$Original, "Type" = sample(dat$Type))
