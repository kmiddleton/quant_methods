...
ggplot(LionNoses, aes(proportion.black, age)) +
  ...
  labs(x = "Proportion Black", y = "Age (yr)")
```
...
```{r}
data("HornedLizards", package = "abd")
...
```{r}
t_obs <- t.test(horn.length ~ group, data = HornedLizards)
t_obs
```
...
for (i in 2:nreps) {
  ...
  t_stats[i] <- t.test(horn.length ~ Rand_group,
...
logspace <- function(d1, d2, n) exp(log(10) * seq(d1, d2,
                                                  length.out = n))
reps_list <- floor(logspace(1, 5, n = 20))
...
for (j in 1:nrow(reps_ex)) {
  ...
  do_rand <- function() {
    Rand_group <- sample(HornedLizards$group)
    as.numeric(t.test(horn.length ~ Rand_group,
           data = HornedLizards)$statistic)
  }
