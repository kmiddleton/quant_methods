...
<center><img src="https://upload.wikimedia.org/wikipedia/commons/c/c5/Leonardo_Design_for_a_Flying_Machine%2C_c._1488.jpg" width="50%" /></center>

## The tidyverse

A set of packages to load, clean, filter, plot, and generally deal with data in a "tidy" way. Largely the work of [Hadley Wickham](http://hadley.nz/) and colleagues.
...
## A few tips for clean code

- In order to make sure your code is reproducible, you want to make sure it is understandable by others (and your future self). To achieve this, your code should be clean, readable, and well annotated. So how do we do that?
...
# alpha = transparancy, smaller is more transparent
ggplot(tvdat, aes(x = log(DD5_TOT), y = sqrt(tv), col = Region)) +
  geom_point(shape = 1, size = 3, alpha = 1/3) +
  geom_smooth(aes(col = Region), method = 'lm') + 
  xlab(bquote('log(Degree Days above' ~5^o~C*')')) +
...
```
<center>
<img src="https://i.imgur.com/1yKQBJZ.png" height="200px" />
</center>
...
`read_excel()` produces `tibble`s, which print a little prettier but are otherwise equivalent and completely compatible.
...
<center><img src="https://i.imgur.com/fDpfRDA.png" width="85%" /></center>
