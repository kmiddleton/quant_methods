...
scatter3D(grid_approx$mu, grid_approx$sigma, grid_approx$log_lik,
          pch = 16,
...
```{r testgl2, webgl=TRUE, echo=FALSE}
plot3d(grid_approx$mu, grid_approx$sigma, grid_approx$log_lik,
       xlab = "mu", ylab = "sigma", zlab = "log-likelihood",
       col = "red")
plot3d(grid_approx$mu[which.max(grid_approx$log_lik)],
       grid_approx$sigma[which.max(grid_approx$log_lik)],
       grid_approx$log_lik[which.max(grid_approx$log_lik)],
       col = "blue",
       size = 10,
       add = TRUE)
```
