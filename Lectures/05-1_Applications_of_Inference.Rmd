...
For a set of $Y_i$ and parameters ($\Theta$; i.e., mean and standard deviation) the likelihood of the model is the product of their individual likelihoods:
...
This is only the likelihood for first observation. We need the likelihoods for all `r length(undulation_rate)` undulation rates to get a model likelihood.
...
Overall likelihood is the product of those likelihoods:
...
For a model in which the mean is 0 and  standard deviation is 1, the model log-likelihood is `r round(log(lik), 3)`.
...
data_frame(
  ...
  labs(y = "Probability Density", x = "value")
```
